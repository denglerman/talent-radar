...
import { CandidateWithSignals, CompanyTier, TIER_LABELS } from '@/types';
...
interface Props {
  ...
  collapsed: boolean;
  onToggle: () => void;
  onAddCandidate: (data: AddCandidateData) => Promise<boolean>;
...
function getSparklineData(signals: { created_at: string }[]): number[] {
  const now = Date.now();
  const buckets = Array(30).fill(0);
  signals.forEach((s) => {
    const daysAgo = Math.floor((now - new Date(s.created_at).getTime()) / (1000 * 60 * 60 * 24));
        ...
        {!collapsed && (
          <div className="flex-1 overflow-y-auto">
            {sorted.map((candidate, i) => {
              const sparkData = getSparklineData(candidate.signals);
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between mb-1">
                    <div className="flex items-center gap-2">
                      <div
                        ...
                        style={{ backgroundColor: getMobilityColor(candidate.mobility_score) }}
                      >
                  ...
                  {/* Company subtitle */}
                  <div className="data-mono text-[10px] text-[#475569] truncate mb-1">
                    {candidate.current_company || 'Unknown'}
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span
                        ...
                        style={{ color: getMobilityColor(candidate.mobility_score) }}
                      >
                        {candidate.mobility_score}
                      </span>
                    </div>
                    <div className="w-16 h-4">
                      <Sparklines data={sparkData} color={getMobilityColor(candidate.mobility_score)} />
                    </div>
                  </div>
                </motion.div>
              );
...
function AddCandidateModal({ onClose, onAdd }: { onClose: () => void; onAdd: (data: AddCandidateData) => Promise<boolean> }) {
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="bg-[#111827] border border-[#1e293b] rounded-xl p-6 w-96 max-h-[80vh] overflow-y-auto"
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="mb-3">
            ...
            <input
              ...
              placeholder="@handle"
            />
            <span className="data-mono text-[9px] text-[#334155] mt-1 block">Without the @ prefix</span>
          </div>
          <div className="mb-3">
            <label className="data-mono text-[10px] text-[#64748b] uppercase tracking-wider block mb-1">
              GitHub Username
